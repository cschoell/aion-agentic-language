...
- [x] Method calls on `List`, `Map`, `Str` � including `reduce`, `any`, `all`, `find`, `zip`, `enumerate`, `sort`, `sort_by`, `reverse`, `flat_map`, `first`, `last`
- [x] First-class functions (`FnVal` with closure capture)
- [x] **Math builtins** � `abs`, `min`, `max`, `pow`, `sqrt`, `floor`, `ceil` (interpreter + bytecode)`n- [x] **Closures / lambda literals** — `fn(x: Int) -> Int { x * 2 }`; full closure semantics; `list.map(fn)` / `list.filter(fn)` dispatch
...
- [x] **231 passing unit tests** across 6 suites: `AionLanguageTest` (18), `BytecodeCompilerTest` (49), `InterpreterQaTest` (5), `SmallFeaturesTest` (39), `ModuleImportTest` (12), `ResourceScriptE2ETest` (60)
- [x] **IntelliJ plugin** (`aion-lang-intellij` module) — syntax highlighting, brace matching, line/block commenting, color settings page for all token types; installable as `.zip` via *Settings → Plugins → Install from disk*
...
- [x] **Refinement type bytecode injection** � `CheckConstraint` instruction emitted by compiler, executed by VM; 5 new bytecode tests
...
- [x] **IntelliJ plugin** — `aion-lang-intellij` Gradle module; `buildPlugin` produces installable ZIP
